...
docker exec -u 0 namenode bash -c "sudo gem install apt-spy2"
docker exec -u 0 namenode bash -c "sudo apt-spy2 fix --commit --launchpad --country=PL"
docker exec -u 0 namenode bash -c "apt-get -y --option="APT::Acquire::Retries=3" update"
docker exec -u 0 namenode bash -c "apt-get -y --option="APT::Acquire::Retries=3" install openssh-client openssh-server"
